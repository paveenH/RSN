...
MODELS=(
    # "llama3|meta-llama/Llama-3.1-8B-Instruct|llama3|8B|0-11-20 4-11-20 neg4-11-20"  # already done
...
# ==================== Run ====================
echo "=================================================="
echo "Capitulation Rate Experiment"
...
for MODEL_CFG in "${MODELS[@]}"; do
    ...
    if [ $? -eq 0 ]; then
        echo ""
        echo "[✓ Done] Capitulation Rate — ${MODEL} ${SIZE} finished at: $(date)"
    else
        echo ""
        echo "[✗ Failed] Capitulation Rate — ${MODEL} ${SIZE}"
        exit 1
    fi
