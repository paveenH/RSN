...
def main():
    ...
    save_dir = os.path.join(f"/data2/paveen/RolePlaying/src/models/components/answer_NMD_role/{model_name}_{alpha}")
    os.makedirs(save_dir, exist_ok=True)

    # Load difference matrices with exception handling
    try:
        data_char_diff = np.load(os.path.join(matrix_path, f'norole_nn_diff_mean_{size}.npy'))       # (1,1,layers,hidden_size)
        data_none_char_diff = np.load(os.path.join(matrix_path, f'none_nn_diff_mean_{size}.npy')) # (1,1,layers,hidden_size)
