...
def main():
    ...
    #    We'll compute average + sample count later
    ...
    # 8) Calculate n, avg_logit, avg_prob for each role
    role_metrics = {}
    for role in roles:
    ...
    for role in roles:
        print(f"Role: {role}, Samples={role_metrics[role]['n']}, "
              f"Avg Logit={role_metrics[role]['logit']}, "
              f"Avg Prob={role_metrics[role]['prob']}")
    
    # 10) Write to CSV
    # Row: single for this task
    # Columns: [Task, {role1}_sample, {role1}_logit, {role1}_prob, {role2}_sample, ...]
    csv_path = os.path.join(save_dir, "logits_summary.csv")
    file_exists = os.path.isfile(csv_path)
    
    # Build header
    # Example: ["Task", "none_{task}_expert_sample", "none_{task}_expert_logit", "none_{task}_expert_prob", ... ]
    header = ["Task"]
    for role in roles:
        safe_role_name = role.replace(" ", "_")  # just in case
        header.append(f"{safe_role_name}_sample")
        header.append(f"{safe_role_name}_logit")
        header.append(f"{safe_role_name}_prob")
    
    # Build row
    row = [task]
    for role in roles:
        row.append(role_metrics[role]["n"])       # sample count
        row.append(role_metrics[role]["logit"])   # average logit
        row.append(role_metrics[role]["prob"])    # average probability
