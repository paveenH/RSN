...
for size, tasks in csv_data_per_size.items():
    ...
    output_csv = os.path.join(output_dir, f"total_counts_{size}.csv")
...
for size, tasks in csv_data_per_size.items():
    ...
    avg_non_char_acc = df["non_char_acc"].mean()
    avg_char_acc = df["char_acc"].mean()

    # Calculate average E count percentage for non-char and char
    avg_none_char_E_pct = df["none_char_E_pct"].mean()
    avg_char_E_pct = df["char_E_pct"].mean()

    
    print(f"Size: {size}")
    print(f"  Average Non-Character Accuracy: {avg_non_char_acc:.2f}%")
    print(f"  Average Character Accuracy: {avg_char_acc:.2f}%")
    print(f"  Average Non-Character E: {avg_none_char_E_pct:.2f}%")
    print(f"  Average Character E: {avg_char_E_pct:.2f}%")
    ...
    fig.legend(
        [h1, h2, h3, h4],
        ['Non-Character Accuracy', 'Character Accuracy', 'Non-Character E Counts', 'Character E Counts'],
    ...
    combined_plot_path = os.path.join(output_dir, f"combined_accuracy_E_counts_{size}.png")
    plt.tight_layout()
